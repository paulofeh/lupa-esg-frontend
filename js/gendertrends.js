class GenderTrendVisualization {
  constructor(data) {
    this.data = data;
    this.initialize();
  }
  ...
  switchTab(selectedTab) {
    ...
    document
      .querySelectorAll(".trend-view")
      .forEach((view) => view.classList.remove("active"));

    selectedTab.classList.add("active");
    const segment = selectedTab.dataset.segment;
    document.querySelector(`#${segment}-trend`)?.classList.add("active");
  }

  createViews() {
    const container = document.querySelector(".trend-views");
    if (!container) {
      console.error("Container .trend-views não encontrado");
      return;
    }
  ...
  createTrendView(segment) {
    ...
    view.id = `${segment}-trend`;

    const visualizationDiv = document.createElement("div");
    visualizationDiv.className = "trend-visualization";
  ...
  drawAllVisualizations() {
    document.querySelectorAll(".trend-view").forEach((view) => {
      view.style.display = "block";
    });

    Object.keys(this.data).forEach((segment) => {
      const container = document.querySelector(
        `#${segment}-trend .trend-visualization`
      );
      if (container) {
        container.innerHTML = "";
  ...
  createSVG(segment, container) {
    ...
    const verticalOffset =
      (-Math.tan((angle * Math.PI) / 180) * (endX - startX)) / 2;
...
window.GenderTrendVisualization = GenderTrendVisualization;
